    ...
    public function destroy(QualityForm $qualityForm)
    {
        ...
        try {
            DB::transaction(function () use ($qualityForm) {
                ...
                if ($qualityForm->campaign->active_form_version_id && 
    ...
    public function importAttributes(Request $request, QualityForm $qualityForm)
    {
        ...
        try {
            while (($row = fgetcsv($handle, 0, ",")) !== FALSE) {
                ...
                if (empty($categoriaName)) continue; // Fila vacía
            ...
            // 1. Validar suma total de categorías
            $totalCatWeight = array_sum(array_column($data, 'weight'));
            if (abs($totalCatWeight - 100) > 0.1) {
                throw new \Exception("Los pesos de las categorías suman $totalCatWeight%, deben sumar 100%.");
            }

            // 2. Validar suma de items por categoría
            foreach ($data as $cat) {
                $totalItemWeight = array_sum(array_column($cat['items'], 'weight_percent'));
                if (abs($totalItemWeight - 100) > 0.1) {
                    throw new \Exception("Los ítems de la categoría '{$cat['name']}' suman $totalItemWeight%, deben sumar 100%.");
                }
            }
