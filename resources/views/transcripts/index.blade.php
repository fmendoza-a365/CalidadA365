    ...
    <div class="card flex flex-col h-[calc(100vh-12rem)] overflow-hidden">
        ...
        <div class="flex-none z-20 relative rounded-t-2xl bg-white dark:bg-[#141414]">
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-800/30 border-b border-gray-100 dark:border-gray-800">
                <form method="GET" action="{{ route('transcripts.index') }}" class="flex flex-col sm:flex-row gap-4 items-end">
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <!-- View -->
                                    <a href="{{ route('transcripts.show', $interaction) }}" class="btn-ghost btn-sm text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400" title="Ver Detalle">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <!-- Download -->
                                    <a href="{{ route('transcripts.download', $interaction) }}" class="btn-ghost btn-sm text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400" title="Descargar Original">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                                        </svg>
                                    </a>
                                    ...
                                    @if(!$interaction->evaluation)
                                        <form method="POST" action="{{ route('transcripts.evaluate', $interaction) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="btn-ghost btn-sm text-amber-600 hover:text-amber-700 dark:text-amber-500 dark:hover:text-amber-400" title="Evaluar con IA">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <!-- Edit -->
                                    <a href="{{ route('transcripts.edit', $interaction) }}" class="btn-ghost btn-sm text-indigo-600 hover:text-indigo-700" title="Editar">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <form id="delete-form-{{ $interaction->id }}" 
                                        method="POST" 
                                        action="{{ route('transcripts.destroy', $interaction) }}" 
                                        class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>

                                    <div x-data="{}" x-on:confirm-action.window="if ($event.detail.name === 'delete-transcript-{{ $interaction->id }}') document.getElementById('delete-form-{{ $interaction->id }}').submit()">
                                        <x-confirm-modal 
                                            name="delete-transcript-{{ $interaction->id }}" 
                                    ...
                                    @if($interaction->evaluation)
                                        <a href="{{ route('evaluations.show', $interaction->evaluation) }}" class="btn-ghost btn-sm text-emerald-600 hover:text-emerald-700" title="Ver Evaluación">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if($interactions->hasPages())
            <div class="flex-none px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#141414] rounded-b-2xl">
                {{ $interactions->links() }}
            </div>
        @endif
    </div>
