        ...
        <aside
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1 scrollbar-thin">
                ...
                @if(auth()->user()->can('view_campaigns') || auth()->user()->can('view_quality_forms') || auth()->user()->can('view_transcripts') || auth()->user()->can('view_insights'))
                    ...
                    @if(auth()->user()->can('view_quality_forms'))
                ...
                @if(auth()->user()->hasRole('admin') || auth()->user()->can('manage_ai_settings'))
                    ...
                    @if(auth()->user()->hasRole('admin'))
                        <a href="{{ route('settings.tts') }}"
                            class="nav-item {{ request()->routeIs('settings.tts*') ? 'nav-item-active' : '' }}"
                            title="Texto a Voz">
                            <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="/docs-executive.html" target="_blank" class="nav-item">
                    <svg class="nav-icon text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                    <span>Manual Ejecutivo <svg class="inline w-3 h-3 ml-1 opacity-50" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
