                        ...
                        <tr>
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold text-sm shadow-sm"
                                        ...
                                        @if($campaign->logo_url)
                                            <img src="{{ $campaign->logo_url }}" alt="" class="w-full h-full rounded-full object-cover">
                                        @else
                            ...
                            <td class="text-center">
                                @if($campaign->activeFormVersion)
                                    <svg class="w-4 h-4 text-emerald-500 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span class="text-sm text-gray-600 dark:text-gray-400 ml-1">v{{ $campaign->activeFormVersion->version_number }}</span>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            <td class="text-center">
                                <a href="{{ route('campaigns.show', $campaign) }}" class="btn-ghost btn-sm" title="Ver Detalles">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @if(auth()->user()->hasRole('admin'))
                                        <a href="{{ route('campaigns.edit', $campaign) }}" class="btn-ghost btn-sm text-indigo-600" title="Editar">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <form id="delete-campaign-form-{{ $campaign->id }}" 
                                            method="POST" 
                                        ...
                                        <div x-data="{}" x-on:confirm-action.window="if ($event.detail.name === 'delete-campaign-{{ $campaign->id }}') document.getElementById('delete-campaign-form-{{ $campaign->id }}').submit()">
                                            <x-confirm-modal 
                                                name="delete-campaign-{{ $campaign->id }}" 
