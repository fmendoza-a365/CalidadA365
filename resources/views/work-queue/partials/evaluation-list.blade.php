...
<div class="space-y-2">
    @forelse($items as $evaluation)
        <a href="{{ route('evaluations.show', $evaluation) }}"
            class="block rounded-xl border border-gray-200 bg-white px-3 py-2.5 transition hover:border-indigo-200 hover:bg-indigo-50/30 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/5">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex-shrink-0 text-right">
                    <div class="text-lg font-bold {{ $scoreTone($evaluation->percentage_score) }}">
                        {{ $evaluation->percentage_score !== null ? number_format((float) $evaluation->percentage_score, 1).'%' : '--' }}
                    </div>
                    <div class="text-xs text-gray-400 dark:text-gray-500">Ver detalle</div>
                </div>
            </div>
        </a>
    @empty
